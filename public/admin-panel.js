...
announcementForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!res.ok) {
      // Handle express-validator array or standard error messages
      if (body.errors && Array.isArray(body.errors)) {
        announcementError.textContent = body.errors
          .map((e) => e.msg)
          .join(", ");
      } else {
        announcementError.textContent =
          body.error || body.message || "Failed to create announcement";
      }
      return;
    }
